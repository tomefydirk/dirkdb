...
mod tests {
    ...
    use crate::general_struct::element::{
        BinOp, CompareOp, Condition, LogicalOp, PrimitiveElement, TableCell, LogicResult,
    };
    ...
    fn test_comparisons() {
        let c = Condition::Comparison {
            left: Box::new(Condition::Primitive(PrimitiveElement::Identifier("x".into()))),
            op: CompareOp::Gt,
        ...
        let c = Condition::Comparison {
            left: Box::new(Condition::Primitive(PrimitiveElement::Identifier("empty".into()))),
            op: CompareOp::Is,
    ...
    fn test_logical() {
        ...
        let right = Condition::Comparison {
            left: Box::new(Condition::Primitive(PrimitiveElement::Identifier("y".into()))),
            op: CompareOp::Lt,
    ...
    fn test_binary_ops() {
        let c = Condition::BinaryOp {
            left: Box::new(Condition::Primitive(PrimitiveElement::Identifier("x".into()))),
            op: BinOp::Add,
            right: Box::new(Condition::Primitive(PrimitiveElement::Identifier("y".into()))),
        };
    ...
    fn test_negate_and_not() {
        let negate = Condition::Negate(Box::new(Condition::Primitive(PrimitiveElement::Number(5.0))));
        assert_eq!(negate.eval(&ctx()), LogicResult::Other(TableCell::Number(-5.0)));

        let not = Condition::Not(Box::new(Condition::Primitive(PrimitiveElement::Number(0.0))));
    ...
    fn test_primitives() {
        ...
        assert_eq!(c.eval(&ctx()), LogicResult::Other(TableCell::String("rust".into())));
